...
- ROLLER Data API is not part of the live customer account flow.
- Cancellation handling is external-event driven in this pilot.

## ROLLER API Usage Split
- ROLLER REST API: live operational API for webhook registration, live booking-related operations, and targeted repair reads.
- ROLLER Data API: read-only export API for periodic extraction to analytics or reconciliation stores.
- Do not use ROLLER Data API for single-customer live account fetches, webhook replacement, or booking/waiver UI refresh.
...
- No ROLLER REST fallback path for failed webhook processing in pilot.
- No ROLLER Data API usage in pilot live paths.
