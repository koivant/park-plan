...
- Use ROLLER REST API only for live operational needs such as webhook management and targeted repair fetches.
- Do not use ROLLER Data API for real-time account updates; reserve it for periodic export or analytics use cases only.
...
- Targeted repair: use ROLLER REST API only for repair or missing live details.
- Periodic export: use ROLLER Data API only for scheduled export/reconciliation jobs, not for single-record live reads ([ROLLER Data API](https://mysupport.roller.software/hc/en-us/articles/360001653475-Data-API), [ROLLER GraphQL](https://docs.roller.app/graphql)).
...
6. Add cache and scheduler-based reconciliation, using ROLLER Data API only if periodic export repair is later required.
