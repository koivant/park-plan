...
## Runtime
The app is a Node.js and TypeScript service using Express. Source files use ECMAScript modules. Runtime configuration is read from environment variables in `config.ts`.
...
ROLLER integration uses OAuth client credentials for REST API calls. MVP ROLLER usage is webhook-first: booking data enters through webhooks, and targeted guest-detail lookup fills missing customer contact fields when needed. The rate limiter spaces ROLLER API calls to one call per second, which stays within ROLLER's published 3600 calls per hour per OAuth token limit.
...
//TODO: There is no home park concept
...
Redis/Memorystore is not implemented. This is optional in the future.
